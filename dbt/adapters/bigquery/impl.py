...
class BigQueryAdapter(BaseAdapter):
    ...
    @available.parse_none
    def update_table_description(self, database: str, schema: str, identifier: str,
                                 description: str = None, labels: dict = None):
        if not (description or labels):
            return
        ...
        with self.connections.exception_handler(query):
            ...
            resp = PartitionsModelResp(unique_id=unique_id,
                                       ...
                                       error=("\n".join(error["message"].strip() for error in job.errors)
                                              if job.errors else None),
                                       dry_run=dry_run,
                                       bytes_billed=job.total_bytes_billed,
                                       bytes_processed=job.estimated_bytes_processed,
